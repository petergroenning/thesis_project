...
struct ode_integration {
	vector<Type> X_next;
	matrix<Type> P_next;
	ode_integration(vector<Type> x0__, matrix<Type> p0__, Type t, Type dt__, int algo__, Type logsigma_x, Type logtheta, Type mu){
		if(algo__==1){
			/*Forward Euler*/
			X_next = x0__ + f__(logtheta, mu, x0__(0)) * dt__;
			P_next = p0__ + (dfdx__(logtheta)*p0__ + p0__*dfdx__(logtheta).transpose() + g__(logsigma_x)*g__(logsigma_x).transpose()) * dt__;
		} else if (algo__==2){
			...
			a1__ = dt__ * (dfdx__(logtheta)*p0__ + p0__*dfdx__(logtheta).transpose() + g__(logsigma_x)*g__(logsigma_x).transpose());
			/*step 2*/
			t = t + 0.5 * dt__;
			x0__ = X0__ + 0.5 * k1__;
			p0__ = P0__ + 0.5 * a1__;
      k2__ = dt__ * f__(logtheta, mu, x0__(0));
      a2__ = dt__ * (dfdx__(logtheta)*p0__ + p0__*dfdx__(logtheta).transpose() + g__(logsigma_x)*g__(logsigma_x).transpose());
			/*step 3*/
			x0__ = X0__ + 0.5 * k2__;
			p0__ = P0__ + 0.5 * a2__;
      k3__ = dt__ * f__(logtheta, mu, x0__(0));
      a3__ = dt__ * (dfdx__(logtheta)*p0__ + p0__*dfdx__(logtheta).transpose() + g__(logsigma_x)*g__(logsigma_x).transpose());
			/*step 4*/
			t = t + 0.5 * dt__;
			x0__ = X0__ + k3__;
			p0__ = P0__ + a3__;
      k4__ = dt__ * f__(logtheta, mu, x0__(0));
      a4__ = dt__ * (dfdx__(logtheta)*p0__ + p0__*dfdx__(logtheta).transpose() + g__(logsigma_x)*g__(logsigma_x).transpose());
			/*ODE UPDATE*/
...
//////////// set initial value ///////////
	 ...
	 for(int i=0 ; i<t.size()-1 ; i++){
		 ...
		 for(int j=0 ; j<N__(i) ; j++){
			 ode_integration<Type> odelist = {x0__, p0__, t(i)+j*dt__(i), dt__(i), algo__, logsigma_x, logtheta, mu};
			 x0__ = odelist.X_next;
			 p0__ = odelist.P_next;
		 }
		 xPrior(i+1) = x0__;
...
//////////// set initial value ///////////
	 ...
	 for(int k=0 ; k < k_step_ahead ; k++){
		 ...
		 for(int j=0 ; j<N__(i+k) ; j++){
			 ode_integration<Type> odelist = {x0__, p0__, t(i+k)+j*dt__(i+k), dt__(i+k), algo__, logsigma_x, logtheta, mu};
			 x0__ = odelist.X_next;
			 p0__ = odelist.P_next;
		 }
