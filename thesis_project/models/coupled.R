make_model<-function(model, type = 'model1'){
    ...
    if (type == 'model1'){
        ...
        model$addSystem(dx7m~dt*((Y7m-x7m)*a+((x6m-x7m)*f1*FbotIn+(x8m-x7m)*(f2*FbotOut))/V7)+sigma_x*dwx7)
        model$addSystem(dx8m~dt*((Y8m-x8m)*a+((x7m-x8m)*f1*FbotIn+(x9m-x8m)*(f2*FbotOut))/V8)+sigma_x*dwx8)
        model$addSystem(dx9m~dt*((Y9m-x9m)*a+((x8m-x9m)*f1*FbotIn+(x10m-x9m)*(f2*FbotOut))/V9)+sigma_x*dwx9)
        model$addSystem(dx10m~dt*((Y10m-x10m)*a+((x9m-x10m)*f1*FbotIn+(x11m-x10m)*(f2*FbotOut)+(Tmid-x10m)*fmid*FmidIn)/V10)+sigma_x*dwx10)
        model$addSystem(dx11m~dt*((Y11m-x11m)*a+((x10m-x11m)*f3*FtopOut+(x12m-x11m)*(f4*FtopIn))/V11)+sigma_x*dwx11)
        model$addSystem(dx12m~dt*((Y12m-x12m)*a+((x11m-x12m)*f3*FtopOut+(x13m-x12m)*(f4*FtopIn))/V12)+sigma_x*dwx12)
        model$addSystem(dx13m~dt*((Y13m-x13m)*a+((x12m-x13m)*f3*FtopOut+(x14m-x13m)*(f4*FtopIn))/V13)+sigma_x*dwx13)
        model$addSystem(dx14m~dt*((Y14m-x14m)*a+((x13m-x14m)*f3*FtopOut+(x15m-x14m)*(f4*FtopIn))/V14)+sigma_x*dwx14)
        model$addSystem(dx15m~dt*((Y15m-x15m)*a+((Ttop-x15m)*(ftop)*(FtopIn)+(X14-x15m)*(f5*FtopOut)+(ambientTemp-x15m)*u)/V15)+sigma_x*dwx15)
        ...
        model$addSystem(dY8m~dt*(x8m-Y8m)*b+sigma_y*dwy8)
        model$addSystem(dY9m~dt*(x9m-Y9m)*b+sigma_y*dwy9)
        model$addSystem(dY10m~dt*(x10m-Y10m)*b+sigma_y*dwy10)
        model$addSystem(dY11m~dt*(x11m-Y11m)*b+sigma_y*dwy11)
        model$addSystem(dY12m~dt*(x12m-Y12m)*b+sigma_y*dwy12)
        model$addSystem(dY13m~dt*(x13m-Y13m)*b+sigma_y*dwy13)
        model$addSystem(dY14m~dt*(x14m-Y14m)*b+sigma_y*dwy14)
        model$addSystem(dY15m~dt*(x15m-Y15m)*b+sigma_y*dwy15)
