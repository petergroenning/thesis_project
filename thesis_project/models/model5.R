...
make_model <- function(data, old_fit){
    ...
    obj$addSystem(dx0m ~ 1/C *  ((x1m - x0m) * (R+(FbotOut)*Cbot) + (15 - x0m)*U) * dt + (Tbot - x0m) * Vbot * (FbotIn) * dt +  sigma_x0 * dw0)
    obj$addSystem(dx1m ~ 1/C * ((x0m - x1m) * (R+(FbotOut)*Cbot) + (x2m - x1m) * R  + (15 - x1m)*U) * dt + sigma_x1 * dw1)
    obj$addSystem(dx2m ~ 1/C *  ((x1m - x2m) * R + (x3m - x2m) * R + (15 - x2m)*U) * dt + sigma_x2 * dw2)
    obj$addSystem(dx3m ~ 1/C *  ((x2m - x3m) * R + (x4m - x3m) * R + (15 - x3m)*U) * dt + sigma_x3 * dw3)
    obj$addSystem(dx4m ~ 1/C *  ((x3m - x4m) * R + (x5m - x4m) * R + (15 - x4m)*U) * dt + sigma_x4 * dw4)
    obj$addSystem(dx5m ~ 1/C *  ((x4m - x5m) * R + (x6m - x5m) * R + (15 - x5m)*U) * dt + sigma_x5 * dw5)
    obj$addSystem(dx6m ~ 1/C *  ((x5m - x6m) * R + (x7m - x6m) * R + (15 - x6m)*U) * dt + sigma_x6 * dw6)
    obj$addSystem(dx7m ~ 1/C *  ((x6m - x7m) * R + (x8m - x7m) * R + (15 - x7m)*U) * dt + sigma_x7 * dw7)
    obj$addSystem(dx8m ~ 1/C *  ((x7m - x8m) * R + (x9m - x8m) * R + (15 - x8m)*U) * dt + sigma_x8 * dw8)
    obj$addSystem(dx9m ~ 1/C *  ((x8m - x9m) * R + (x10m - x9m) * R + (15 - x9m)*U) * dt + sigma_x9 * dw9)
    obj$addSystem(dx10m ~ 1/C * ((x9m - x10m) * R + (x11m - x10m) *  R + (15 - x10m)*U) * dt +  sigma_x10 * dw10)
    obj$addSystem(dx11m ~ 1/C * ((x10m - x11m) * R + (x12m - x11m) * R + (15 - x11m)*U) * dt  +  sigma_x11 * dw11)
    obj$addSystem(dx12m ~ 1/C * ((x11m - x12m) * R + (x13m - x12m) * R + (15 - x12m)*U) * dt + sigma_x12 * dw12)
    obj$addSystem(dx13m ~ 1/C * ((x12m - x13m) * R + (x14m - x13m) * R + (15 - x13m)*U) * dt + sigma_x13 * dw13)
    obj$addSystem(dx14m ~ 1/C * ((x13m - x14m) * R + (x15m - x14m) * (R+(FtopIn-FtopOut)*Ctop) + (15 - x14m)*U) * dt + sigma_x14 * dw14)
    obj$addSystem(dx15m ~ 1/C * ((x14m - x15m) * (R+(FtopIn-FtopOut)*Ctop)+(ambientTemp-x15m)*Utop) * dt + (Ttop - x15m) * Vtop * (FtopIn-FtopOut) * dt + sigma_x15 * dw15)
    ...
    obj$setParameter(U = c(init=getParam('U', init, old_fit), lb = lb, ub = ub))
...
save(fit,file='models/oldCTSMfit/model5.RData')
