make_model<-function(model){
    ...
    # model$setParameter(Y=c(init=0,lb=-1e2,ub=1e2))

    model$addInput("X1","X2","X3","X4","X5","X6","X7","X8","X9","X11","X12","X13","X14","dFtopIn","dFtopOut","dFtop","dFtopVol","dFmid","dFmidVol","dFbot","dFbotVol","dFbotIn","dFbotOut","dFmidIn","dFmidOut","dFbotIn","dFbotOut")
    #####SETPARAMETERS########
    #SystemNoise
    ...
    model$setParameter(c=c(init=1,lb=-1e2,ub=1e2))
    model$setParameter(d=c(init=1,lb=-1e2,ub=1e2))
