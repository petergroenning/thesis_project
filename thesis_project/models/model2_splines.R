...
make_model <- function(data, old_fit){
    ...
    obj$addSystem(dx0m ~ 1/(c1*b1x0+c2*b2x0+c3*b3x0+c4*b4x0+c5*b5x0) *  ((x1m - x0m) * R) * dt + (Tbot - x0m) * Vbot * FbotIn * dt +  sigma_x0 * dw0)
    obj$addSystem(dx1m ~ 1/(c1*b1x1+c2*b2x1+c3*b3x1+c4*b4x1+c5*b5x1) *  ((x0m - x1m) * R + (x2m - x1m) * R ) * dt + sigma_x1 * dw1)
    ...
    obj$addSystem(dx14m ~ 1/(c1*b1x14+c2*b2x14+c3*b3x14+c4*b4x14+c5*b5x14) * ((x13m - x14m) * R + (x15m - x14m) * R) * dt + sigma_x14 * dw14)
    obj$addSystem(dx15m ~ 1/(c1*b1x15+c2*b2x15+c3*b3x15+c4*b4x15+c5*b5x15) * ((x14m - x15m) * R+(ambientTemp-x15m)*Utop) * dt + (Ttop - x15m) * Vtop * FtopIn * dt + sigma_x15 * dw15)
    ...
    obj$setParameter(Utop = getParam('Utop', init, old_fit))
...
# Make model object
load('models/oldCTSMfit/model2.RData')
...
save(fit,file='models/oldCTSMfit/model2_splines.RData')
...
plot(bs[,1]*c1+bs[,2]*c2+bs[,3]*c3+bs[,4]*c4+bs[,5]*c5,type='l',col='red')
