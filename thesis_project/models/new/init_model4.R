...
model <- ctsm$new()
    ...
    model$addSystem(dx0m~dt/V0*((x1m-x0m)*(FbotOut*exp(f1))+(Tbot-x0m)*(FbotIn*exp(vbot)))+exp(sigma_x)*dwx0)
    model$addSystem(dx1m~dt/V1*((x2m-x1m)*(FbotOut*exp(f2))+(x0m-x1m)*(FbotIn*exp(f2)))+exp(sigma_x)*dwx1)
    model$addSystem(dx2m~dt/V2*((x3m-x2m)*(FbotOut*exp(f2))+(x1m-x2m)*(FbotIn*exp(f2)))+exp(sigma_x)*dwx2)
    model$addSystem(dx3m~dt/V3*((x4m-x3m)*(FbotOut*exp(f3))+(x2m-x3m)*(FbotIn*exp(f3)))+exp(sigma_x)*dwx3)
    model$addSystem(dx4m~dt/V4*((x5m-x4m)*(FbotOut*exp(f3))+(x3m-x4m)*(FbotIn*exp(f3)))+exp(sigma_x)*dwx4)
    model$addSystem(dx5m~dt/V5*((x6m-x5m)*(FbotOut*exp(f4))+(x4m-x5m)*(FbotIn*exp(f4)))+exp(sigma_x)*dwx5)
    model$addSystem(dx6m~dt/V6*((x7m-x6m)*(FbotOut*exp(f4))+(x5m-x6m)*(FbotIn*exp(f4)))+exp(sigma_x)*dwx6)
    model$addSystem(dx7m~dt/V7*((x8m-x7m)*(FbotOut*exp(f5))+(x6m-x7m)*(FbotIn*exp(f5)))+exp(sigma_x)*dwx7)
    model$addSystem(dx8m~dt/V8*((x9m-x8m)*(FbotOut*exp(f5))+(x7m-x8m)*(FbotIn*exp(f5)))+exp(sigma_x)*dwx8)
    model$addSystem(dx9m~dt/V9*((x10m-x9m)*(FbotOut*exp(f6))+(x8m-x9m)*(FbotIn*exp(f6)))+exp(sigma_x)*dwx9)
    model$addSystem(dx10m~dt/V10*((x11m-x10m)*(FtopIn*exp(f6))+(x9m-x10m)*(FtopOut*exp(f6))+(Tmid-x10m)*FmidIn*exp(vmid))+exp(sigma_x)*dwx10)
    model$addSystem(dx11m~dt/V11*((x12m-x11m)*(FtopIn*exp(f7))+(x10m-x11m)*(FtopOut*exp(f7)))+exp(sigma_x)*dwx11)
    model$addSystem(dx12m~dt/V12*((x13m-x12m)*(FtopIn*exp(f7))+(x11m-x12m)*(FtopOut*exp(f7)))+exp(sigma_x)*dwx12)
    model$addSystem(dx13m~dt/V13*((x14m-x13m)*(FtopIn*exp(f8))+(x12m-x13m)*(FtopOut*exp(f8)))+exp(sigma_x)*dwx13)
    model$addSystem(dx14m~dt/V14*((x15m-x14m)*(FtopIn*exp(f8))+(x13m-x14m)*(FtopOut*exp(f8)))+exp(sigma_x)*dwx14)
    model$addSystem(dx15m~dt/V15*((x14m-x15m)*(FtopOut*exp(f9))+(Ttop-x15m)*(FtopIn*exp(vtop)))+exp(sigma_x)*dwx15)
...
save(fit, file = 'models/base_model4.Rdata')
