...
make_model <- function(data, old_fit){
    ...
    obj$addSystem(dx0m ~ 1/(c1*b1x0+c2*b2x0+c3*b3x0+c4*b4x0+c5*b5x0) *  ((x1m - x0m) * (R+(FbotOut)*Cbot) + (xs - x0m)*U0) * dt + (Tbot - x0m) * Vbot * (FbotIn) * dt +  sigma_x0 * dw0)
    obj$addSystem(dx1m ~ 1/(c1*b1x1+c2*b2x1+c3*b3x1+c4*b4x1+c5*b5x1)*  ((x0m - x1m) * (R+(FbotOut)*Cbot) + (x2m - x1m) * R  + (xs - x1m)*U1) * dt + sigma_x1 * dw1)
    obj$addSystem(dx2m ~ 1/(c1*b1x2+c2*b2x2+c3*b3x2+c4*b4x2+c5*b5x2) *  ((x1m - x2m) * R + (x3m - x2m) * R + (xs - x2m)*U2) * dt + sigma_x2 * dw2)
    obj$addSystem(dx3m ~ 1/(c1*b1x3+c2*b2x3+c3*b3x3+c4*b4x3+c5*b5x3) *  ((x2m - x3m) * R + (x4m - x3m) * R + (xs - x3m)*U3) * dt + sigma_x3 * dw3)
    obj$addSystem(dx4m ~ 1/(c1*b1x4+c2*b2x4+c3*b3x4+c4*b4x4+c5*b5x4) *  ((x3m - x4m) * R + (x5m - x4m) * R + (xs - x4m)*U4) * dt + sigma_x4 * dw4)
    obj$addSystem(dx5m ~ 1/(c1*b1x5+c2*b2x5+c3*b3x5+c4*b4x5+c5*b5x5) *  ((x4m - x5m) * R + (x6m - x5m) * R + (xs - x5m)*U5) * dt + sigma_x5 * dw5)
    obj$addSystem(dx6m ~ 1/(c1*b1x6+c2*b2x6+c3*b3x6+c4*b4x6+c5*b5x6) *  ((x5m - x6m) * R + (x7m - x6m) * R + (xs - x6m)*U6) * dt + sigma_x6 * dw6)
    obj$addSystem(dx7m ~ 1/(c1*b1x7+c2*b2x7+c3*b3x7+c4*b4x7+c5*b5x7) *  ((x6m - x7m) * R + (x8m - x7m) * R + (xs - x7m)*U7) * dt + sigma_x7 * dw7)
    obj$addSystem(dx8m ~ 1/(c1*b1x8+c2*b2x8+c3*b3x8+c4*b4x8+c5*b5x8) *  ((x7m - x8m) * R + (x9m - x8m) * R + (xs - x8m)*U8) * dt + sigma_x8 * dw8)
    obj$addSystem(dx9m ~ 1/(c1*b1x9+c2*b2x9+c3*b3x9+c4*b4x9+c5*b5x9) *  ((x8m - x9m) * R + (x10m - x9m) * R + (xs - x9m)*U9) * dt + sigma_x9 * dw9)
    obj$addSystem(dx10m ~ 1/(c1*b1x10+c2*b2x10+c3*b3x10+c4*b4x10+c5*b5x10) * ((x9m - x10m) * R + (x11m - x10m) *  R + (xs - x10m)*U10) * dt +  sigma_x10 * dw10)
    obj$addSystem(dx11m ~ 1/(c1*b1x11+c2*b2x11+c3*b3x11+c4*b4x11+c5*b5x11) * ((x10m - x11m) * R + (x12m - x11m) * R + (xs - x11m)*U11) * dt  +  sigma_x11 * dw11)
    obj$addSystem(dx12m ~ 1/(c1*b1x12+c2*b2x12+c3*b3x12+c4*b4x12+c5*b5x12) * ((x11m - x12m) * R + (x13m - x12m) * R + (xs - x12m)*U12) * dt + sigma_x12 * dw12)
    obj$addSystem(dx13m ~ 1/(c1*b1x13+c2*b2x13+c3*b3x13+c4*b4x13+c5*b5x13) * ((x12m - x13m) * R + (x14m - x13m) * R + (xs - x13m)*U13) * dt + sigma_x13 * dw13)
    obj$addSystem(dx14m ~ 1/(c1*b1x14+c2*b2x14+c3*b3x14+c4*b4x14+c5*b5x14) * ((x13m - x14m) * R + (x15m - x14m) * (R+(FtopIn-FtopOut)*Ctop) + (xs - x14m)*U14) * dt + sigma_x14 * dw14)
    ...
    obj$addInput("Ttop", "Tbot", "FtopIn", "FtopOut", "FbotIn", "FbotOut", "ambientTemp", "Tmid", "FmidIn", "FmidOut")
    ...
    obj$setParameter(Vmid = c(init=getParam('Vmid', 1, old_fit), lb = lb, ub = ub))
    ...
    obj$setParameter(U0 = c(init=getParam('U0', init, old_fit), lb = lb, ub = ub))
    obj$setParameter(U1 = c(init=getParam('U1', init, old_fit), lb = lb, ub = ub))
    obj$setParameter(U2 = c(init=getParam('U2', init, old_fit), lb = lb, ub = ub))
    obj$setParameter(U3 = c(init=getParam('U3', init, old_fit), lb = lb, ub = ub))
    obj$setParameter(U4 = c(init=getParam('U4', init, old_fit), lb = lb, ub = ub))
    obj$setParameter(U5 = c(init=getParam('U5', init, old_fit), lb = lb, ub = ub))
    obj$setParameter(U6 = c(init=getParam('U6', init, old_fit), lb = lb, ub = ub))
    obj$setParameter(U7 = c(init=getParam('U7', init, old_fit), lb = lb, ub = ub))
    obj$setParameter(U8 = c(init=getParam('U8', init, old_fit), lb = lb, ub = ub))
    obj$setParameter(U9 = c(init=getParam('U9', init, old_fit), lb = lb, ub = ub))
    obj$setParameter(U10 = c(init=getParam('U10', init, old_fit), lb = lb, ub = ub))
    obj$setParameter(U11 = c(init=getParam('U11', init, old_fit), lb = lb, ub = ub))
    obj$setParameter(U12 = c(init=getParam('U12', init, old_fit), lb = lb, ub = ub))
    obj$setParameter(U13 = c(init=getParam('U13', init, old_fit), lb = lb, ub = ub))
    obj$setParameter(U14 = c(init=getParam('U14', init, old_fit), lb = lb, ub = ub))

    obj$setParameter(xs = c(init=getParam('xs', 20, old_fit), lb = 10, ub = 30))
...
obj$setOptions(list("maxNumberOfEval" = 1000, "eps" = 1e-8))
fit <- obj$estimate(data = data[interval,], firstorder = TRUE, threads = 16)

save(fit,file='models/oldCTSMfit/model4_splines.RData')
