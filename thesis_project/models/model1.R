...
make_model <- function(data, old_fit){
    ...
    obj$addSystem(dx15m ~ 1/C* ((x14m - x15m) * R) * dt + (Ttop - x15m) * Vtop * FtopIn * dt + sigma_x15 * dw15)
    ...
    # obj$setParameter(Uloss = c(init = getParam('Uloss', init, old_fit), lb = lb, ub = ub))
    ...
    # obj$setParameter(xs = c(init = getParam('xs', 20, old_fit), lb = lb, ub = ub))
...
save(fit,file='models/oldCTSMfit/model1.RData')
