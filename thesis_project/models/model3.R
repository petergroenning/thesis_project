...
make_model <- function(data, old_fit){
    ...
    obj$addSystem(dx0m ~ 1/C *  ((x1m - x0m) * (R+FbotOut*Cbot)) * dt + (Tbot - x0m) * Vbot * FbotIn * dt +  sigma_x0 * dw0)
    obj$addSystem(dx1m ~ 1/C *  ((x0m - x1m) * (R+FbotOut*Cbot) + (x2m - x1m) * R ) * dt + sigma_x1 * dw1)
    ...
    obj$addSystem(dx14m ~ 1/C * ((x13m - x14m) * R + (x15m - x14m) * (R+FtopOut*Ctop)) * dt + sigma_x14 * dw14)
    obj$addSystem(dx15m ~ 1/C* ((x14m - x15m) * (R+FtopOut*Ctop)+(ambientTemp-x15m)*Utop) * dt + (Ttop - x15m) * Vtop * FtopIn * dt + sigma_x15 * dw15)
...
interval<-8500:9000
# Make model object
load('models/oldCTSMfit/model3.RData')
...
save(fit,file='models/oldCTSMfit/model3.RData')
